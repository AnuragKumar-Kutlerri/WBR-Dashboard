...
function parseLoyalty(wb) {
  ...
  if (sms.length > 28) {
    const smsWoW = [];
    // The WoW header row carries the two week labels (e.g. "Week of May 25" /
    // "Week of May 18") in cols 2-3 — capture them so the UI isn't hardcoded.
    let smsCols = null;
    for (let i = 19; i <= 29; i++) {
    ...
    loyMarketing = { smsWoW, smsCols, email7d, email30d };
  }
...
export function parseWeekFolder(folderPath, weekLabel) {
  ...
  // Read the matched files into buffers and hand off to the shared buffer parser
  // so folder-sourced and blob-sourced weeks run identical parsing logic.
  const read = f => f ? fs.readFileSync(path.join(folderPath, f)) : null;
  return parseWeekBuffers(
    { wbr: read(wbrFile), loyalty: read(loyFile), catering: read(catFile) },
    label,
  );
}

// ── Public: parse a week from in-memory buffers ─────────────────────────────
// `buffers` holds up to three optional Buffers: { wbr, loyalty, catering }.
// Any subset may be present; missing workbooks yield empty sections. Used by
// the Blob-backed data route (Vercel filesystem is read-only at runtime).
export function parseWeekBuffers(buffers, weekLabel) {
  const { wbr, loyalty: loyBuf, catering: catBuf } = buffers || {};
  ...
  if (wbr) {
    const wb = XLSX.read(wbr, { type: 'buffer', cellDates: true });
    const W = parseWBR(wb);
    weekly = W.weekly || {};
    ptd = W.ptd || {};
    ytd = W.ytd || {};
  ...
  if (loyBuf) {
    const wb = XLSX.read(loyBuf, { type: 'buffer', cellDates: true });
    const L = parseLoyalty(wb);
    loyalty = L.loyalty || {};
    if (L.loyaltyMarketing) loyaltyMarketing = L.loyaltyMarketing;
  }
  if (catBuf) {
    const wb = XLSX.read(catBuf, { type: 'buffer', cellDates: true });
